...
# One last thing the first row of our data set is actually the sum of Scotland so we do not want it there 
...
# We can combine datsets that have shared data using the merge() function. Looking at the authority_data and SIMD data frames, the authority and Council_area columns have the same data. 
...
Merged_data$Deprivation <-ifelse(Merged_data$SIMD>=median(Merged_data$SIMD),"least","most")# We are going to need one more binary variable that will divide our data set in most deprived/less deprived based on the median
